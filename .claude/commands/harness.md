...
- **단계별 진행** — 질문하거나 답변할 때 한 번에 많은 내용을 제시하지 않는다. 먼저 핵심을 간단히 요약한 뒤 하나씩 진행한다. 질문도 가능한 한 한 번에 하나씩 묻고, 사용자의 확인 또는 답변 후 다음 단계로 넘어간다.
- **예외** — 사용자가 명시적으로 복수의 자료·비교표·목록 정리 등을 요청한 경우, 또는 작업 수행을 위해 여러 개의 필수 파라미터를 한 번에 확인해야 하는 경우에는 묶어서 제공할 수 있다.
...
2. **자기완결성** — 각 step 파일은 독립된 Codex 실행에서 처리된다. "이전 대화에서 논의한 바와 같이" 같은 외부 참조는 금지한다. 필요한 정보는 전부 파일 안에 적는다.
