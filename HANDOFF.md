...
- [DB결함] **구현·SMD DB 반영·제품 브랜치 push 완료 / 코드·DB 학습 가이드 재구성 완료 / 제품 PR 미생성** — `origin/main` `fb3c240` → `feature-control-0713` `2aad3db`의 실질 diff는 커밋 `88bf7dc` 4파일이다. 원본 가이드는 Git과 현재 PC에서 확보하지 못해 2026-08-01 현재 코드·handoff 근거로 `Desktop\Tomes\PR\[07.31 DB결함 코드 학습 가이드]`의 MD·HTML 재구성본을 만들고 복원 한계와 미실행 검증을 명시했다. 폐기한 `[07.29 DB결함 코드 이해 리뷰]`는 재사용하지 않는다 — [상세](handoff/DB결함.md)
