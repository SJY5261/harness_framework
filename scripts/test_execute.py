...
def tmp_project(tmp_path):
    """phases/, Codex 정본, docs/ 를 갖춘 임시 프로젝트 구조."""
    phases_dir = tmp_path / "phases"
    phases_dir.mkdir()

    agents_md = tmp_path / "AGENTS.md"
    agents_md.write_text("# Rules\n- rule one\n- rule two", encoding="utf-8")
    project_rules = tmp_path / "PROJECT_RULES.md"
    project_rules.write_text("# Project Rules\n- safety first", encoding="utf-8")
...
class TestBuildPreamble:
    ...
    def test_forbids_direct_commit(self, executor):
        result = executor._build_preamble("", "")
        assert "git 커밋을 직접 실행하지 마라" in result
...
class TestInvokeCodex:
    def test_invokes_codex_with_correct_args(self, executor):
        ...
        assert ["--sandbox", "workspace-write"] == cmd[2:4]
        assert "--json" in cmd
