...
- 제품 저장소는 `feature-materialmap-0803`이며, 2026-08-03에 사용자 지시에 따라 다른 기능 작업만 작업 단위별 stash로 분리했다. 2026-08-04 관리용 CRUD를 제거하고 대분류 상속·단일 PK·테이블명 변경을 반영한 현재 기능 변경은 SQL 1파일을 포함해 6파일 `+292/-10`이다. `.claude/settings*.json`과 `shell/runDev.local.bat`은 로컬 환경 파일이며 제품 커밋은 만들지 않았다.
...
- 범위 축소 당시 제품 diff는 `SystemServiceImpl.java`, `system.xml`, 공통·페이지 JSP, 탭 JS의 5파일 `+60/-10`이었다. 이후 대분류 상속 SQL과 단일 PK·테이블명 전환을 포함한 현재 diff는 6파일 `+292/-10`이다.

## 다음 행동

1. 신형 의미 코드 불일치 13건을 새 매핑으로 이관할지, 기존값 예외로 인정할지 결정한다.
2. 개발 DB의 직접 매핑 백업과 PK 변경 전 백업 테이블은 배포·복구 판단이 끝난 뒤 별도 승인을 받아 제거한다.
