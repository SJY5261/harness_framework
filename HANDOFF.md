...
- PR 설명·PR리뷰·사용자 전달 보고서는 `Desktop\PR\[MM.DD 요약]` 작업 폴더에 MD 정본과 자체완결 HTML 정확히 2개만 만들며 `아티팩트.html`·`아티팩트.url` 등 별도 아티팩트 파일은 만들지 않는다. 일반 보고서는 `현상 → 원인 → 해결 → 채택 이유 → 결과` 순서를 사용한다. Sites는 사용자가 웹 게시를 명시적으로 요청한 경우에만 게시하고 URL은 답변으로 전달한다. DB 컬럼·인덱스·제약·프로시저·함수·트리거 변경은 변경 전후, 실제 DDL/DML, 적용 순서, 검증·복구 SQL을 리뷰에 필수로 남긴다. `UserPromptSubmit` 규칙 신선도 훅은 변경 알림을 공식 JSON 출력 형식으로 반환하도록 보정했다.
