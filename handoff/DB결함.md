...
- `C:\Users\User\Desktop\PR\[07.31 DB결함 코드 학습 가이드]`의 원본은 Git·현재 PC·휴지통·현재 Codex 세션·7/22 이관 백업에서 발견되지 않았다. 2026-08-01 현재 코드와 handoff를 근거로 원본과 구분되는 **재구성본** MD 정본과 자체완결 HTML 두 파일을 만들었다. 수주→작업지시 흐름, 번호·내부키, 핵심 10개 테이블, 논리 조인, 동시채번·이중발행·사본동기화·삭제전파, `CONTROL_NUM` 백업·백필·`NOT NULL` DDL, 프로시저·트리거·복구·검증 절차를 포함한다.
