...
- [SiteAdmin_재질표면처리매핑] **1~7단계 구현·개발 DB 125건 반영·격리 전체 빌드 완료 / UI·기존 데이터 정책 대기** — `feature-materialmap-0803` 작업트리에는 재질-표면처리 관련 수정 9파일과 SQL 1파일만 남겼고 다른 작업·로컬 설정은 5개 독립 stash로 분리했다. 구형 D03 데이터 18,954건과 신형 매핑 불일치 13건, Site Admin 코드 정본 29↔51 불일치는 8단계 전 결정이 필요하다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
- [Codex주에이전트전환] **현재 PC `work` 프로필·핵심 개발·테스트 기능 호환 완료 / GUI 도구 데이터 이관 대기** — 저장소 밖 로컬 마커와 `scripts/pc_profile.ps1`로 PC를 구분하며, `%클라우드`의 Claude 로컬 설정과 개발 실행 배치는 각각 독립 stash로 보관했다. 프로필 테스트 4 PASS, 하네스 76 PASS이며 기존 개발·테스트 기능과 Git 원격·Gradle도 정상이다. VS Code·Codex 개인 홈·SourceTree·DBeaver·Git 전역 데이터 이관은 별도 대기 — [상세](handoff/Codex주에이전트전환.md)
