...
- [SiteAdmin_재질표면처리매핑] **대분류 상속 DB·조회·필터·ABBR 참조·전체 빌드 완료 / 기존 데이터 정책·PR 문서 재갱신 대기** — `TBL_MATERIAL_SURFACE_MAP`의 12개 대분류·38건은 유지하면서 참조 정본을 D03 `ABBR_NM`으로 전환했다. 유효하지 않은 ABBR 0건, 기존 접두어 방식과 결과 차이 0건, 하위 D03 확장 125건을 확인했다. 제품 변경은 SQL 포함 6파일 `+314/-10`이며 기존 PR 설명·코드 가이드에는 이번 참조 전환을 아직 반영하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
