...
- [KAN-46_사이트설정부서종류] **E04 실제 그리드 교체·1920×1080 원격 목업 비율 조정 완료 / 세부 기능 협의 대기** — `common-code-master.jsp`의 `commonCodeBotColModel`과 `/getCommonCodeList` 조회를 응용해 그룹코드 E04 `E04R10~E04R20` 11건을 표시한다. 외부 Chrome 디버깅 탭을 3번 모니터 `1920×1080`·브라우저 100%로 고정하고 원격 `dev-smd`·로컬 `smd.localhost`를 동시에 비교했다. 원격 목업 컬럼 영역 1433px을 로컬에 적용한 결과 8개 열 오차는 모두 0px이다. SQL 변경 없이 `git diff --check`와 전체 Gradle 빌드를 통과했다. 부가기능은 사용자 지적 후 항목별로 진행한다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
