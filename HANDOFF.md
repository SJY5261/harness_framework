...
- [SiteAdmin_재질표면처리매핑] **로그인 테넌트 우선·BASIC 후퇴 정정, D03 REF_CD 후퇴 제거, 대분류 상속 DB·조회·필터·ABBR 참조·전체 빌드 완료 / 기존 데이터 정책·PR 문서 재갱신 대기** — `selectSessionCodeList`가 실제 `#{LOGIN_SYSTEM_ID}`의 대분류 매핑을 우선하도록 바로잡고 불필요한 D02 조인을 제거했으며, 캐시를 `v2`로 갱신했다. 화면 D03 필터는 `MATERIAL_TYPE_CDS`만 사용하고 다른 기준코드만 기존 `REF_CD`를 유지한다. 분기 시나리오·전체 빌드·18081 Spring/MyBatis 로딩을 통과했고 제품 변경은 SQL 포함 6파일 `+310/-10`이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
