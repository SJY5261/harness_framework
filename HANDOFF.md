...
- [가공확정] **KAN-6 공통 작업상세 표 재구현·전체항목 테스트 데이터 완료 / 배지·상태 제외 로직·기타 디바이스 보류** — 공통 `bottom.jsp` 데스크톱 표 1파일만 `+4/-8`로 수정했고, 개발 DB에 작업지시번호 `QA-ITEM-DETAIL-FULL-001` 1건을 생성해 24개 표시값이 모두 채워짐을 확인했다. 표시 진행상태는 `출하`, 원본 상태는 `S53R46`이며 `S53R14` 이력은 없다. 전용 seeder·검증·cleanup은 `%테스트` `7c21ad6`에 push했다 — [상세](handoff/가공확정.md)
