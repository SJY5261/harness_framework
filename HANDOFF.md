...
- [DB결함] **실동작 5 PASS·1 FAIL / 개발 DB ORDER_SEQ 시퀀스 조치·재검증 대기** — 작업번호 동시발번, 이중발행 차단, 기존 납기·접수번호 전파, 바코드 삭제 전파, 자동채번 Y 고정은 통과했다. 신규 주문은 네임드 락 이후 `SEQ_CONTROL_PART_ORDER`가 기존 PK 92015018·92015019와 충돌해 HTTP 500으로 실패했다. 테스트 데이터 잔존은 0건이며 제품 변경은 미커밋 상태다 — [상세](handoff/DB결함.md)
