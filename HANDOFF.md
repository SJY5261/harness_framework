...
- [가공확정] **R14/R16 생성·판정 의존 제거·작업상세 배지 적용 / NULL 후속작업 취소 복귀 결함 실증 / DB 이관 보류** — `PART_STATUS=NULL`에서 CAM·MCT 시작은 각각 S53R26/S53R28로 정상 전이하지만, 취소하면 이력만 삭제되고 현재값이 그대로 남는다. 재확정처럼 과거 이력이 있으면 NULL 대신 과거 S53 상태로 잘못 복원되는 것도 실제 API로 확인했다. 테스트 데이터는 기존 7분기 최종 상태로 복원했으며 제품 코드는 사용자 결정 전 수정하지 않았다 — [상세](handoff/가공확정.md)
