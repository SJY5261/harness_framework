...
- 클라우드의 현재 공통 정책은 `BASIC`과 `USE_YN`으로 운용하더라도 테넌트 민감 정책을 `BASIC` 전용으로 하드코딩하지 않는다. 실제 `SYSTEM_ID`에 따른 테넌트별 설정과 공통값 후퇴가 가능한 경계를 유지해 향후 온프라미스에서도 같은 기능을 재사용할 수 있게 한다.
- 아직 요구되지 않은 온프라미스 전용 화면·설정·배포 구조까지 선구현하지 않고, 현재 기능 범위 안에서 테넌트 식별·정책 조회·기본값 결정 지점을 분리한다.
