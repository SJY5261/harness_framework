...
- [SiteAdmin_재질표면처리매핑] **1~7단계 구현·개발 DB 125건 반영·격리 빌드 완료 / 신규 매핑 캐시 무효화·UI·기존 데이터 정책 대기** — DB와 실행 코드는 정상이지만 직접 시드 전 Redis 응답과 브라우저 30분 캐시가 남아 화면이 신규 `MATERIAL_TYPE_CDS` 대신 기존 `REF_CD` 매핑으로 후퇴하는 현상을 재현했다. 기능 변경은 재질-표면처리 수정 9파일과 SQL 1파일만 남았고 다른 기능은 3개 stash로 분리했다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
