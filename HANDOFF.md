...
- [KAN-46_사이트설정부서종류] **13px 스크롤 보정·직접 체크·ON 색상·추가/삭제 팝업 UI 완료 / DB 영속화·사용 건수 조회 협의 대기** — E04 11건 그리드의 컬럼 합계를 세로 스크롤 반영 폭 1420px로 비례 축소해 가로 넘침을 0px로 만들고, 행 선택과 체크 상태를 분리했다. 사용 ON은 녹색으로 복구했으며 추가 버튼 문구와 JIRA 첨부 기준 추가·삭제 확인·사용 중 삭제 불가 팝업 흐름을 실화면에서 검증했다. 현재 V2는 미리보기 전용이고 E04 응답에 사용 건수와 영속 삭제 API가 없어 추가·삭제는 화면 상태까지만 반영한다. 제품 로컬 브랜치는 `main` 동일 커밋에서 `sunPro/KAN-46~54`를 각각 생성했고, 현재 작업은 `sunPro/KAN-46`이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
