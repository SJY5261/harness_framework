...
- PR 리뷰 기본 산출물은 작업 폴더의 MD 정본과 자체완결 HTML 2종이며, Codex Sites 게시와 `아티팩트.url`은 사용자가 웹 게시를 명시적으로 요청한 경우에만 추가한다. `UserPromptSubmit` 규칙 신선도 훅은 변경 알림을 공식 JSON 출력 형식으로 반환하도록 보정했다.
