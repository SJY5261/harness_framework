...
- [SiteAdmin_재질표면처리매핑] **대분류 상속 DB·조회·필터·캐시 세대·전체 빌드·분리 서버 기동·문서 완료 / 기존 데이터 정책 대기** — 개별 D03 예외를 허용하지 않는 정책에 맞춰 `NA` 포함 12개 대분류·38건 매핑으로 전환했다. 하위 D03 확장 125건은 기존 직접 매핑과 양방향 차이 0건이며, 기존 테이블은 백업 이름으로 보존했다. 제품 변경은 SQL 포함 6파일 `+278/-10`이고 PR 설명·초보자용 코드 가이드 MD·HTML을 갱신했다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
