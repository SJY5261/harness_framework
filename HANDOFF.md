...
- PR 리뷰 기본 산출물은 작업 폴더의 MD 정본과 자체완결 HTML 2종이며, Codex Sites 게시와 `아티팩트.url`은 사용자가 웹 게시를 명시적으로 요청한 경우에만 추가한다. DB 컬럼·인덱스·제약·프로시저·함수·트리거 변경은 변경 전후, 실제 DDL/DML, 적용 순서, 검증·복구 SQL을 리뷰에 필수로 남긴다. `UserPromptSubmit` 규칙 신선도 훅은 변경 알림을 공식 JSON 출력 형식으로 반환하도록 보정했다.
...
- [DB결함] **구현·제품 브랜치 push·PR 리뷰 MD/HTML·DB 변경 SQL 보완 완료 / 독립 검토 대기** — `%클라우드` 커밋 `88bf7dc`를 push했고, `Desktop\PR\[07.28 DB결함 수정 리뷰]`에 `CONTROL_NUM` 백필·DDL과 `SP_CONTROL_BATCH` 가드·검증·복구 SQL을 추가했다. 현재 DB와 백업 프로시저의 `CUSTOMER_REV` 차이 때문에 백업 전체 복구는 금지하고 선택적 역패치로 교정했다. 기존 Sites 버전 7은 이력으로만 보존하고 이번 보완은 재게시하지 않음 — [상세](handoff/DB결함.md)
