...
- 사용자가 요청한 1~7단계(DB 구조·시드·백엔드·공통코드 응답·필터·캐시·검증)는 완료했다. Site Admin 화면 구현과 업무 화면 실동작 반영은 이번 범위에 포함하지 않았다.
...
- 2026-08-03 실제 화면에서 소재종류 선택 후 신규 매핑이 아니라 기존 `REF_CD` 매핑이 표시되는 현상을 재현했다. DB와 실행 코드는 정상이지만 직접 DB 시드 전 생성된 Redis 공통코드 캐시가 남아 `MATERIAL_TYPE_CDS` 없는 응답을 반환하고, 브라우저 30분 캐시가 이를 다시 보관한 것이 원인이다. 캐시 무효화와 재검증은 아직 수행하지 않았다.
...
- 미완료: Site Admin 화면의 적용소재 편집 UI와 업무 화면 실브라우저 검증은 8단계 이후 작업이다.
...
1. `getSessionCodeList` Redis 캐시 전체와 브라우저 공통코드 `dsCache`를 안전하게 무효화한 뒤 신규 `MATERIAL_TYPE_CDS` 응답과 소재별 표면처리를 재검증한다.
2. 배포 후 같은 문제가 반복되지 않도록 공통코드 응답 구조 변경 시 브라우저 캐시 키 버전 상승 또는 구형 캐시 자동 폐기 중 하나를 적용할지 결정한다.
3. 신형 의미 코드 불일치 13건을 새 매핑으로 이관할지, 기존값 예외로 인정할지 결정한다.
4. Site Admin 화면이 참조하는 구형 `TBL_BASIC_CODE` D03 29개와 런타임 `TBL_CODE(BASIC)` D03 51개의 정본 불일치를 해소한다.
5. 8단계로 Site Admin 적용소재 편집 UI를 구현하고 저장 성공 시 브라우저 캐시 무효화를 호출한다.
...
- 직접 DB 시드 후 Redis와 브라우저 공통코드 캐시가 무효화되지 않아 현재 화면은 신규 매핑 대신 기존 `REF_CD` 매핑을 사용한다.
