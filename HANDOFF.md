...
- [KAN-46_사이트설정부서종류] **추가·단일삭제 DB 연동·기본값 잠금·단일 체크·헤더 정렬 완료 / 사용여부 목업 ON/OFF·DB 연동 보류** — 기존 `insertCommonCode`·`insertCommonLangCode`를 재사용하되 `TBL_CODE.SYSTEM_ID`도 세션 `LOGIN_SYSTEM_ID`로 강제했다. E04 조회에 로그인 테넌트의 `TBL_USER.DEPARTMENT` 참조 건수를 포함하고, 참조 0건인 로그인 테넌트 직접추가 코드만 기존 `InnodaleDao.deleteGrid()`와 `deleteCommonCodeAdmin`·`deleteCommonCodeLangAdmin` queryId를 변경 없이 재사용해 트랜잭션으로 단일 삭제한다. BASIC 행의 체크박스와 사용여부는 비활성화하고 직접추가 행은 한 번에 하나만 선택되며 컬럼 헤더는 가운데 정렬한다. 로컬 SMD에서 `E04R21·E04R22`를 팝업으로 실제 추가하고 단일 선택 전환·삭제한 뒤 두 테이블 잔존 0건을 확인했다. 사용여부는 행 메모리 `DEL_YN`만 전환하고 DB 저장은 하지 않으며 직접추가 수정은 후속 대기다. 현재 제품 브랜치는 `sunPro/KAN-46`이고 제품 변경은 미커밋이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
