...
- 2026-08-03 실제 화면에서 소재종류 선택 후 신규 매핑이 아니라 기존 `REF_CD` 매핑이 표시되는 현상을 재현했다. 직접 DB 시드 전 Redis 응답과 브라우저 30분 캐시가 원인이었으며, 서버·브라우저 공통코드 캐시 키를 `surface-material-map-v1`으로 전환하고 전체 빌드를 통과했다. 실행 중인 8081 서버 재시작과 실화면 재검증은 사용자 진행 대기다.
...
- 현재 8081 JVM은 변경 전인 19:09 시작 프로세스이므로 새 캐시 키 적용에는 서버 재시작이 필요하다. Codex는 서버를 종료하거나 재시작하지 않았다.

## 다음 행동

1. 8081 서버를 재시작하고 브라우저를 새로고침한 뒤 `/json-list`의 D03 51개가 `MATERIAL_TYPE_CDS`를 포함하는지와 소재별 표면처리를 재검증한다.
...
- 캐시 키 전환 코드는 빌드됐지만 현재 실행 중인 8081 JVM에는 아직 로드되지 않았다. 서버 재시작 전 화면은 기존 `REF_CD` 매핑을 계속 사용할 수 있다.
