...
- [SiteAdmin_재질표면처리매핑] **DB·백엔드·필터·캐시 키 전환·전체 빌드·실화면 연결·PR 설명·코드 가이드 완료 / UI·매핑 구조·기존 데이터 정책 대기** — 매핑 조회 API 125건, D03 51/51 신규 매핑과 소재별 필터를 최신 실화면에서 재확인하고 비DB 코드 9파일의 PR 설명과 변경 전후 코드 가이드 MD·HTML을 만들었다. 남은 범위는 직접 매핑과 대분류 상속 방식 결정, Site Admin 편집 UI와 기존 데이터·코드 정본 정책이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
