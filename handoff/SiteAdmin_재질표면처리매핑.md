...
- 2026-08-20 KAN-46 부서 삭제의 신규 `deleteDepartmentCode`·`deleteDepartmentCodeLang` queryId와 KAN-46 전용 `SystemDao` 메서드를 모두 제거하고, 기존 `InnodaleDao.getInfoNumberValue()`·`InnodaleDao.deleteGrid()` 및 `deleteCommonCodeAdmin`·`deleteCommonCodeLangAdmin` queryId를 변경 없이 재사용하도록 정리했다. 서비스가 `HIGH_CD=E04`, `SEL_SYSTEM_ID=LOGIN_SYSTEM_ID`를 강제하고 `TBL_USER.DEPARTMENT` 참조 건수 0건을 확인한 뒤 두 기존 삭제 쿼리를 트랜잭션으로 실행한다. 조작된 요청값 `HIGH_CD=D01`, `SEL_SYSTEM_ID=BASIC`으로도 경영지원 12건과 BASIC 행 삭제가 차단됐으며, 기존 SMD 직접추가 1건은 보존하고 다음 발번의 임시 코드를 실제 추가·삭제해 검증 잔존 0건과 최종 E04 12건(BASIC 11·SMD 1)을 인증 브라우저에서 확인했다. 실행 중 서버의 파일 잠금으로 첫 빌드가 `processResources` 정리 단계에서 실패했지만 해당 개발 서버만 재시작한 뒤 전체 Gradle 빌드를 통과했다.
