...
- [가공확정] **KAN-63 화면·기존 데이터 보존 / 메뉴·R14/R16 코드 비활성화 / 후속 판단 기준 S11 전환·PR 검증 중** — 작업지시 확정·취소의 기존 S53 생성은 유지하고 자재·POP·도면·MCT·검사의 판단 기준만 동시에 저장되는 S11으로 전환했다. 원격 커밋 `53f5f3f` 위에 V1 XML 보정 3파일이 로컬에 남아 있으며 전체 빌드·XML·JS·diff 검증을 통과했다 — [상세](handoff/가공확정.md)
