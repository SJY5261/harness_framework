...
- [SiteAdmin_재질표면처리매핑] **1~7단계 구현·개발 DB 125건 반영·격리 전체 빌드 완료 / UI·기존 데이터 정책 대기** — `feature-materialmap-0803`의 다른 기능 작업은 단가검토·KAN-63·KAN-6의 3개 독립 stash로 분리했고, 로컬 설정·실행 배치는 작업트리에 유지했다. 기능 변경은 재질-표면처리 수정 9파일과 SQL 1파일만 남았다. 기존 데이터·코드 정본 정책은 8단계 전 결정이 필요하다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
