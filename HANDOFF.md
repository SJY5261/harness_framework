...
- [가공확정] **R14/R16 생성·판정 의존 제거·작업상세 27종 배지 적용 / 핵심 10/10·확장 18 PASS / DB 이관 보류** — 공통 데스크톱 작업상세는 S53 22종과 S11 후퇴 5종을 코드 기준 7개 배지 그룹으로 표시한다. 8081 재기동 후 실제 데이터 5색과 렌더러 입력 2색을 확인했고, `PART_STATUS` 필드가 빠진 구버전 응답은 초록색으로 오인하지 않고 텍스트로 후퇴한다. R14/R16은 과거값 표시용 배지 매핑에만 남고 생성·게이트에는 사용하지 않는다. 공유 DB 현재 2,214건·이력 8,207건은 이관 승인 전 변경하지 않는다 — [상세](handoff/가공확정.md)
