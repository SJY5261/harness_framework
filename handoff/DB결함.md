...
_최종 상태: 2026-08-10 `%테스트` 실동작 결과 작업번호 동시발번, 동일 주문 이중발행 차단, 기존 납기·접수번호 전파 범위, 바코드 삭제 전파, 자동채번 Y 고정은 통과했다. 신규 주문은 코드 락이 아니라 개발 DB의 `SEQ_CONTROL_PART_ORDER`가 기존 `TBL_ORDER.ORDER_SEQ`와 충돌해 HTTP 500으로 실패하므로 DB 시퀀스 정합성 조치와 재검증이 남아 있다. 제품 변경은 `feature-control-0713` 작업트리 5파일에 미커밋으로 유지하며, 긴 진행 이력은 [역사 보존본](../logs/handoff/DB결함_2026-07-28_history.md)에 있다._
...
- `C:\Users\User\Desktop\PR\[08.10 DB결함 실동작 테스트]`에는 2026-08-10 실동작 결과와 DB 시퀀스 충돌·백업 테이블 미확인 경고를 정리한 MD 정본과 자체완결 HTML 두 파일만 있다.
...
- `%테스트` 자동 로그인 브라우저와 개발 DB `smd`로 실제 API를 호출했다. 기능 판정은 5 PASS / 1 FAIL이다.
...
- 신규 주문 동시 2건은 `GET_LOCK`과 `RELEASE_LOCK`이 순차 성공했지만 `orderMapper.createOrderAuto`가 `ORDER_SEQ` 92015018·92015019의 기존 PK와 충돌해 HTTP 500으로 실패했다. 기존 활성 충돌 구간은 92015014~92015021이다.
- 접수번호 실패와 독립적으로 하위 기능을 확인하기 위해 빈 QA 전용 ORDER_SEQ 두 건을 사용했으며, 종료 후 주문·작업·매핑·진행이력·바코드를 삭제해 확인한 잔존 건수는 모두 0이다. 사용된 DB 시퀀스 값은 되돌리지 않아 공백은 남는다.
- 과거 복구 지점 `SP_CONTROL_BATCH_BAK_260720`은 존재하지만 `BAK_TBL_CONTROL_CTRLNUM_260720`은 현재 DB에서 확인되지 않았다. 삭제 경위 확인과 시퀀스 조치는 공유 DB 변경 승인 전까지 수행하지 않는다.
