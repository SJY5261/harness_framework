...
- 2026-08-19 KAN-46 `사이트 설정 > 부서 종류` 화면설계 동작을 `%클라우드`의 `/commonCodeMasterV2`에 구현했다. Jira 본문과 로컬 첨부 `p077.jpg`·`p078.jpg`가 파일명·용량까지 일치함을 확인하고 스크린샷을 정본으로 사용했다. 목록 4건과 동적 건수, `ST01xx` 자동 코드, 직접추가 행 강조, 행 더블클릭 수정, 사용여부 전환, 체크박스 선택·전체 선택, 필수값 인라인 검증, 미사용 행 영구삭제 확인, `ST0101` 사용자 데이터 12건 삭제 차단을 브라우저 내 동작으로 연결했다. ParamQuery 11이 Grid 밖 Delete 키에도 존재하지 않는 Grid 인스턴스의 `clear()`를 호출하는 전역 핸들러 결함은 플러그인을 수정하지 않고 이 팝업 입력의 Delete 키 전파만 차단해 회피했다. 제공된 테스트 계정의 인증 세션과 1600×1000 Chrome에서 초기→추가→수정→삭제 가능→삭제 불가 및 빈 부서명·SORT 차단을 모두 통과했고 JavaScript 오류는 0건이며 캡처는 `D:\LOGS\screenshots\KAN-46_site-department_20260819.png`에 있다. 티켓에 DB 테이블·저장 API·사용 건수 산정 쿼리가 정의되지 않아 화면설계 범위의 비영속 동작만 구현했고 DB 변경은 하지 않았다.
