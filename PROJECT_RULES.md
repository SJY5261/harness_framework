...
- 현재 개발 대상인 클라우드는 회사 공통 규격에 고객이 맞추는 제품이고, 온프라미스는 고객별 요구에 제품이 맞추는 형태다.
- 클라우드의 현재 공통 정책은 `BASIC`과 `USE_YN`으로 운용하더라도 테넌트 민감 정책을 `BASIC` 전용으로 하드코딩하지 않는다. 실제 `SYSTEM_ID`에 따른 테넌트별 설정과 공통값 후퇴가 가능한 경계를 두고, 현재 코드부터 클라우드와 온프라미스 양쪽에서 활용할 수 있는 범용 구조로 구현한다.
- 향후 온프라미스를 인수할 때 공통 기능을 그대로 가져가거나 최소한의 정책 확장만으로 적용할 수 있게 테넌트 식별·정책 조회·기본값 결정 지점을 분리한다. 다만 아직 확인되지 않은 고객별 세부 요구까지 추측해 화면·설정·배포 기능을 선구현하지는 않는다.
