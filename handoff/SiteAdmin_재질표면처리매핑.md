...
- 제품 저장소는 `feature-materialmap-0803`이며, 2026-08-03에 사용자 지시에 따라 다른 작업과 로컬 설정을 작업 단위별 stash로 분리했다. 현재 작업트리에는 이번 재질-표면처리 관련 수정 9파일과 신규 SQL 1파일만 남았고 제품 커밋은 만들지 않았다.
...
- Claude 로컬 설정: `43e0228dbec0782fc9e365429bcf6fb11d8d094f`
- 로컬 개발 실행 배치: `ecb530590222131bda5537c751c03b1c3abd366b`
...
- 현재 작업트리에는 staged 파일이 없고, 위 관련 파일만 수정·미추적 상태로 남아 있다.
...
4. 안전한 시점에 8081 개발 서버를 재시작한 뒤 신규 API 응답과 업무 화면 필터를 실브라우저에서 검증한다.
...
- 신규 Java·매퍼 코드는 현재 실행 중인 8081 서버에 아직 로드되지 않았다.
