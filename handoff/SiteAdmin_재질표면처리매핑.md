...
- 2026-08-04 같은 9파일의 변경 전후 코드, 계층별 호출 흐름, API 계약, 캐시 3계층, 리뷰 위험을 설명한 코드 가이드 MD·자체완결 HTML을 `C:\Users\User\Desktop\PR\[08.04 재질표면처리 코드 변경 가이드]`에 생성했다. MD와 HTML은 제목 33개·표 6개·코드 블록 18개가 일치하고 HTML 공통 스타일은 현재 `templates/standalone-report.html` 정본과 동일하다.
...
- 현재 작업트리의 9파일 `+266/-10`을 다시 측정하고 Controller → Service → DAO → MyBatis → 공통코드 응답 → 화면 필터·캐시 흐름을 파일·현재 줄 위치별로 정리했다.
- 확인된 잠재 문제인 저장 API 역할 제한 부재, 관리 UI 미연결, 빈 매핑 저장 불가와 코드상 추론되는 테넌트 매핑 코드명 `NULL` 가능성을 구현 완료 항목과 분리해 기록했다.
- 산출물 폴더에는 동일 이름의 MD·HTML 두 파일만 두었다. HTML은 정본 스타일 완전 일치, UTF-8, 외부 자산 0, 표 가로 스크롤 6개, Edge 헤드리스 렌더를 확인했다.
