...
- [KAN-44_소재표면처리기준정보] **D01 소재 PQ Grid 11 실조회 완료 / 필터·테넌트 ON/OFF 저장 대기** — `sunPro/common-code-master`의 `/commonCodeMasterV2` 소재 표를 개발 DB D01 활성 78건 원격 조회 그리드로 교체했다. 구버전 매퍼를 사용하던 8081 서버 재기동 후 인증 화면에서 API 200·78건·JavaScript 오류 0건을 확인했다. `TBL_TENANT_CODE_USE`는 설계 결정 전이라 이번 조회·표시에는 사용하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
