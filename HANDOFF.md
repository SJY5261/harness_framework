...
- [DB결함] **구현·SMD DB 반영·제품 브랜치 push·PR 리뷰와 신입 개발자용 코드·DB 학습 가이드 완료 / 제품 PR 미생성** — `origin/main` `fb3c240` → `feature-control-0713` `2aad3db`의 실질 diff는 커밋 `88bf7dc` 4파일이며 clean build PASS. `Desktop\PR\[07.31 DB결함 코드 학습 가이드]`는 수주→작업지시 흐름과 번호·내부키·10개 테이블의 핵심 컬럼·실제 FK·이중발행·사본동기화·삭제·DB계약을 MD/HTML로 설명한 새 정본이다. 폐기한 `[07.29 DB결함 코드 이해 리뷰]`는 재사용하지 않는다 — [상세](handoff/DB결함.md)
