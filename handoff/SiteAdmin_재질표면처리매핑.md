...
- 2026-08-05 사용자와 팀장 조언으로 위 테넌트 우선·`BASIC` 후퇴 구조를 재확인했다. 현재 클라우드는 `BASIC`과 `USE_YN` 중심으로 운영하지만, 향후 고객별 요구 중심의 온프라미스를 인수하더라도 같은 재질-표면처리 기능을 별도 복제하지 않도록 테넌트 정책 경계를 유지한다.
- 이는 온프라미스 전용 관리 기능을 지금 추가한다는 뜻이 아니다. 현재 조회 범위에서는 실제 `SYSTEM_ID` 정책 조회와 공통값 후퇴만 보장하고, 관리 UI·설정 범위는 별도 요구가 생길 때 결정한다.
