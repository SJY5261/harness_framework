...
1. 최신 `v3` 코드로 개발 서버를 재기동하고 인증 세션에서 D03 `MATERIAL_TYPE_CDS`를 재조회한다.
2. 2026-08-05 `ABBR_NM` 참조 전환과 2026-08-06 환경별 `SYSTEM_ID` 조회를 기존 PR 설명·코드 가이드 MD·HTML에 반영한다.
3. 신형 의미 코드 불일치 13건을 새 매핑으로 이관할지, 기존값 예외로 인정할지 결정한다.
4. 개발 DB의 직접 매핑 백업과 PK 변경 전 백업 테이블은 배포·복구 판단이 끝난 뒤 별도 승인을 받아 제거한다.
5. 향후 관리 화면이 별도로 요청될 때만 구형 `TBL_BASIC_CODE` D03 29개와 런타임 `TBL_CODE(BASIC)` D03 51개의 정본 불일치를 먼저 해소하고 대분류 단위로 설계한다.
