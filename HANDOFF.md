...
- [KAN-46_사이트설정부서종류] **13px 스크롤 보정·직접 체크·ON 색상·추가/삭제 팝업 UI 완료 / 단일 삭제·기본값 읽기 전용·직접추가 CRUD 규칙 확정 / 사용여부 목업 ON/OFF 완료·DB 연동 보류** — E04 11건 그리드의 컬럼 합계를 세로 스크롤 반영 폭 1420px로 비례 축소해 가로 넘침을 0px로 만들고, 행 선택과 체크 상태를 분리했다. 사용 ON은 녹색으로 복구했으며 추가 버튼 문구와 JIRA 첨부 기준 추가·삭제 확인·사용 중 삭제 불가 팝업 흐름을 실화면에서 검증했다. XML의 `TBL_CODE` 추가는 `CODE_CD`를 입력값 그대로 저장하며 자동 발번은 없다. 기본값은 수정·삭제하지 않고, 발번 규칙으로 생성한 직접추가 행만 수정·단일 삭제하며 탭 전환 후에도 각 그리드를 유지하는 구현이 대기 중이다. 사용여부는 그리드 행의 `DEL_YN`만 로컬로 전환해 목업 ON/OFF와 탭 전환 상태 유지까지 동작하며 DB 저장은 하지 않는다. 제품 로컬 브랜치는 `main` 동일 커밋에서 `sunPro/KAN-46~54`를 각각 생성했고, 현재 작업은 `sunPro/KAN-46`이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
