...
- [Codex주에이전트전환] **공개 포크 민감정보 차단·Windows 훅 수정·D: CLI 전환 완료 / 기존 자격증명 교체·C: 실행 세션 종료 후 자동 삭제 대기** — 공개 포크의 신규 민감정보를 커밋·푸시 전에 막고, `UserPromptSubmit`의 Windows 이중 PowerShell을 제거해 실제 중첩 경로 실행 테스트를 통과했다. Codex CLI는 D: `0.145.0`만 사용자 `Path`에 남겼으며 기존 공개 이력의 원격 DB 자격증명 교체는 별도 결정이 필요하다 — [상세](handoff/Codex주에이전트전환.md)
