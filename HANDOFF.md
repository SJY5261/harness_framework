...
- [SiteAdmin_재질표면처리매핑] **대분류 상속 DB·조회·필터·단일 PK·테이블명·전체 빌드·8081 기동·문서 완료 / 기존 데이터 정책 대기** — 정본을 `TBL_MATERIAL_SURFACE_MAP`으로 변경하고 `MAP_SEQ` 단일 PK와 시스템·대분류·재질 UNIQUE를 적용했다. `NA` 포함 12개 대분류·38건은 백업과 차이 없고 하위 D03 확장 125건도 기존 직접 매핑과 양방향 차이 0건이다. 2026-08-05 개발 DB `TBL_CODE(BASIC)`의 D01·D03 `ABBR_NM` 접두어 불일치 45건을 동기화하고 변경 전 원본을 별도 백업했다. 제품 변경은 SQL 포함 6파일 `+292/-10`이고 PR 설명·초보자용 코드 가이드 MD·HTML을 갱신했다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
