...
- [SiteAdmin_재질표면처리매핑] **환경별 BASIC·로그인 SYSTEM_ID 조회 전환, D03 REF_CD 후퇴 제거, 대분류 상속 DB·조회·필터·ABBR 참조·전체 빌드 완료 / 최신 서버 재기동·기존 데이터 정책·PR 문서 재갱신 대기** — `selectSessionCodeList`는 `SYSTEM_ID IN ('BASIC', #{LOGIN_SYSTEM_ID})`으로 클라우드의 `BASIC` 또는 온프레미스의 실제 테넌트 매핑을 조회하며, 두 값이 같은 DB에 공존하지 않는 환경별 데이터 정책을 전제로 한다. 변경 전 응답을 재사용하지 않도록 서버·브라우저 캐시는 `v3`로 갱신했다. 화면 D03 필터는 `MATERIAL_TYPE_CDS`만 사용하고 다른 기준코드만 기존 `REF_CD`를 유지한다. 전체 빌드와 JavaScript 문법 검증을 통과했고 제품 변경은 SQL 포함 6파일 `+302/-10`이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
