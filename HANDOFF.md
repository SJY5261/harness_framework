...
- [SiteAdmin_재질표면처리매핑] **다대다 DB 매핑·조회·필터·캐시 세대·전체 빌드·문서 완료 / 매핑 구조·기존 데이터 정책 대기** — 사용자 요구에 맞춰 관리용 목록·저장 API와 Service·DAO CRUD를 제거하고, 기존 공통코드 조회와 화면 필터만 5파일 `+60/-10`으로 남겼다. 격리 전체 빌드와 최종 PR 설명·초보자용 코드 가이드 MD·HTML 갱신을 완료했으며, 남은 범위는 직접 매핑과 대분류 상속 방식 및 기존 데이터 정책 결정이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
