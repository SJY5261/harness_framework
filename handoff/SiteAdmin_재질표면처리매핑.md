...
- 개발 DB `smd`에 `TBL_SURFACE_TREAT_GROUP_MATERIAL_MAP`을 생성하고 `BASIC` 12개 대분류의 D02 매핑 38건을 반영했다. 하위 D03으로 펼치면 기존 직접 매핑 125건과 양방향 차이 0건이다. `NA`는 정식 대분류로 포함하며 활성 재질 5종 전체를 허용한다.
...
- 제품 저장소는 `feature-materialmap-0803`이며, 2026-08-03에 사용자 지시에 따라 다른 기능 작업만 작업 단위별 stash로 분리했다. 2026-08-04 관리용 CRUD를 제거하고 대분류 상속으로 전환한 현재 기능 변경은 SQL 1파일을 포함해 6파일 `+278/-10`이다. `.claude/settings*.json`과 `shell/runDev.local.bat`은 로컬 환경 파일이며 제품 커밋은 만들지 않았다.
...
- 2026-08-04 DB 변경을 포함한 최종 6파일의 대분류 상속 PR 설명 MD·HTML을 `C:\Users\User\Desktop\PR\[08.03 재질표면처리 코드 변경 PR]`에 갱신했다. 제품 변경은 아직 미커밋이고 실제 GitHub PR은 만들지 않았다.
- 2026-08-04 초보 개발자용 코드 가이드도 `대분류 테이블 → MyBatis 조회 → 서버 캐시 → 공통·페이지·탭 화면` 순서와 최종 6파일의 전체 Git diff로 다시 작성했다. 산출물은 `C:\Users\User\Desktop\PR\[08.04 재질표면처리 코드 변경 가이드]`의 MD·자체완결 HTML이다.
...
- 별도 `TBL_SURFACE_TREAT_GROUP_MATERIAL_MAP` 사용 — 정책 단위가 개별 D03이 아니라 `AN`, `PL`, `NA` 같은 대분류이므로 중복 없이 상속 관계를 표현한다.
- 같은 대분류의 모든 하위 D03은 동일한 재질 규칙을 상속하며 개별 예외·추가·제외·덮어쓰기를 허용하지 않는다.
- `NA`는 정식 대분류이며 5개 활성 재질 전체를 허용한다.
- `BASIC` 매핑을 기본값으로 사용하고 같은 대분류의 테넌트 매핑이 있으면 이를 우선하는 조회 구조를 채택했다.
...
- 신규 정본 테이블: `TBL_SURFACE_TREAT_GROUP_MATERIAL_MAP`
...
| 항목 | 변경 전 | 변경 후 |
|---|---|---|
| 재질-표면처리 관계 | 개별 D03 직접 매핑 | `SYSTEM_ID + SURFACE_TREAT_GROUP_CD + MATERIAL_TYPE_CD` 복합 PK |
| 역방향 조회 | `IX_STMM_MATERIAL` | `IX_STGMM_MATERIAL(SYSTEM_ID, MATERIAL_TYPE_CD, SURFACE_TREAT_GROUP_CD)` |
...
6. 재현 SQL과 검증·복구 쿼리는 제품 저장소 `docs/db/20260731_surface_treat_material_map.sql`에 보존했다.

## 구현

- `SystemServiceImpl`: 직접 매핑 Redis 값을 재사용하지 않도록 `getSessionCodeList` 캐시 키에 `surface-material-group-map-v1` 적용
- `system.xml`: 기존 `selectSessionCodeList`가 D03 코드 앞 2자리 대분류의 `MATERIAL_TYPE_CDS`를 집계하고 테넌트 우선·`BASIC` 후퇴를 대분류 단위로 적용
...
- 대분류 상속 확정 후 최종 작업트리의 6파일 `+278/-10`을 다시 측정하고 `대분류 테이블 → MyBatis 조회 → 서버 캐시 → 공통·페이지·탭 화면` 순서로 재구성했다. 신규 DAO·DTO·Service·Controller가 필요 없는 이유를 기존 공통코드 경로와 함께 설명했다.
...
- 범위 축소 당시 제품 diff는 `SystemServiceImpl.java`, `system.xml`, 공통·페이지 JSP, 탭 JS의 5파일 `+60/-10`이었다. 이후 대분류 상속 SQL을 포함한 현재 diff는 6파일 `+278/-10`이다.

## 다음 행동

1. 신형 의미 코드 불일치 13건을 새 매핑으로 이관할지, 기존값 예외로 인정할지 결정한다.
2. 개발 DB의 직접 매핑 백업 테이블은 배포·복구 판단이 끝난 뒤 별도 승인을 받아 제거한다.
3. 향후 관리 화면이 별도로 요청될 때만 구형 `TBL_BASIC_CODE` D03 29개와 런타임 `TBL_CODE(BASIC)` D03 51개의 정본 불일치를 먼저 해소하고 대분류 단위로 설계한다.
