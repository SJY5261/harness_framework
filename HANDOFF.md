...
- [SiteAdmin_재질표면처리매핑] **대분류 상속 DB·조회·필터·ABBR 참조·전체 빌드·DB-Excel 현황 문서 완료 / 기존 데이터 정책·PR 문서 재갱신 대기** — `TBL_MATERIAL_SURFACE_MAP`의 12개 대분류·38건은 유지하면서 참조 정본을 D03 `ABBR_NM`으로 전환했다. 클라우드의 `BASIC` 기본값을 유지하되 향후 온프라미스 재사용을 위해 실제 `SYSTEM_ID` 정책 우선·공통값 후퇴 구조를 유지하기로 재확인했다. 최신 저장 Excel과 개발 DB 520건의 코드 집합·ABBR는 일치하며 직접 불일치는 `E12R10` 코드명 1건이다. 제품 변경은 SQL 포함 6파일 `+314/-10`이며 기존 PR 설명·코드 가이드에는 이번 참조 전환을 아직 반영하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
