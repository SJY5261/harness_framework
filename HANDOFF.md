...
- [Codex주에이전트전환] **D: 실행 정본·현재 VS Code CLI 조회 전환 완료 / 실행 중 도구 데이터 자동 이관 대기** — VS Code 터미널 `PATH`·`CODEX_INSTALL_DIR`와 기존 C: 호환 shim을 D: Codex로 고정해 현재 세션의 `codex-cli 0.145.0` 실행을 확인했다. Playwright·npm·pip·Python 사용자 기반·GitHub CLI·JetBrains 데이터는 D: 이관 완료, VS Code·Copilot·Codex·SourceTree·DBeaver·Gradle·Git 데이터는 해당 프로세스 종료 후 숨김 작업이 마무리한다 — [상세](handoff/Codex주에이전트전환.md)
