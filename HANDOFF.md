...
- [KAN-46_사이트설정부서종류] **E04 실제 그리드 교체·비율 조정 완료 / 세부 기능 협의 대기** — `/commonCodeMasterV2` 목업 표를 `[system][기준정보관리]` 그룹코드 E04의 기존 PQ Grid 조회로 교체해 `E04R10~E04R20` 11건과 `부서 종류` 컬럼을 표시한다. 외부 Chrome 디버깅 탭에서 컬럼 영역 1105px을 측정해 목업과 같은 비율로 적용했고 `git diff --check`와 전체 Gradle 빌드를 통과했다. 추가·수정·삭제 등 세부 기능은 사용자와 합의한 후 진행한다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
