...
- [KAN-44_소재표면처리기준정보] **D01 소재 PQ Grid 11 1차 연결 완료 / 필터·테넌트 ON/OFF 저장 대기** — `sunPro/common-code-master`의 `/commonCodeMasterV2` 소재 목업 표를 개발 DB D01 활성 78건 원격 조회 그리드로 교체했다. `TBL_TENANT_CODE_USE`는 설계 결정 전이라 이번 조회·표시에는 사용하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
