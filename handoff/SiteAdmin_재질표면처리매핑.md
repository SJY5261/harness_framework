...
- 2026-08-19 KAN-46 `사이트 설정 > 부서 종류`는 사용자가 정한 진행 템플릿에 맞춰 1차 작업을 다시 정리했다. 목업의 정적 4건 `ST01xx` 표를 `[system][기준정보관리]` 그룹코드 E04의 기존 PQ Grid 조회(`/getCommonCodeList`, `systemMapper.selectCommonCodeList`, `HIGH_CD=E04`)로 교체했다. 테넌트 `SYSTEM_ID`를 도메인 앞 문자열에서 취하는 현재 시스템 조건을 지켜 `http://smd.localhost:8081`로 접속했고, `E04R10~E04R20` 11건(경영지원~시스템관리자)과 `부서 종류` 컬럼이 실제로 표시되는 것을 확인했다. 1600px 외부 Chrome 디버깅 탭에서 그리드 외곽 1114.67px·컬럼 영역 1105px을 DevTools로 직접 측정하고, PQ Grid 스크롤 영역을 제외한 `83/94/143/147/153/122/178/185px`로 목업과 같은 컬럼 비율을 적용했다. `git diff --check`와 CRLF·UTF-8(no BOM), `%클라우드` 전체 Gradle 빌드를 통과했다. 추가·수정·삭제 등 기타 세부 기능은 사용자와 대화하며 확정하기 전까지 확장하지 않는다.
