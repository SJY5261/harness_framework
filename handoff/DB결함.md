...
_최종 상태: 2026-08-01 원격 브랜치·제품 diff·라이브 DB·테스트의 2026-07-31 기록을 유지한다. 원본 코드 학습 가이드는 현재 PC와 Git에서 확보하지 못해 코드·handoff 근거로 재구성했다. 긴 진행 이력은 [역사 보존본](../logs/handoff/DB결함_2026-07-28_history.md)에 있다._
...
- `C:\Users\User\Desktop\Tomes\PR\[07.31 DB결함 코드 학습 가이드]`의 원본은 Git·현재 PC·휴지통·현재 Codex 세션·7/22 이관 백업에서 발견되지 않았다. 2026-08-01 현재 코드와 handoff를 근거로 원본과 구분되는 **재구성본** MD 정본과 자체완결 HTML 두 파일을 만들었다. 수주→작업지시 흐름, 번호·내부키, 핵심 10개 테이블, 논리 조인, 동시채번·이중발행·사본동기화·삭제전파, `CONTROL_NUM` 백업·백필·`NOT NULL` DDL, 프로시저·트리거·복구·검증 절차를 포함한다.
...
- 2026-08-01 재구성본은 MD 614줄, 제목 58개, 표 행 95개, 코드 펜스 28개이며 MD·HTML 정확히 2파일, 외부 의존성 0, UTF-8 BOM 없음, 원본 미확보·재구성 표시를 확인했다. 라이브 DB와 브라우저 실화면은 재실행하지 않았음을 문서에 명시했다.
