...
- `C:\Users\User\Desktop\PR\[07.31 DB결함 코드 학습 가이드]`에는 수정 코드를 처음 보는 사람 기준으로 수주→작업지시 전체 흐름과 번호 생성·이중발행 방지·사본 동기화·삭제 전파·DB 계약을 대분류한 새 MD 정본과 자체완결 HTML 두 파일만 있다. 자동발번 정책 판단, 프로시저 실제 추가 블록, `CONTROL_NUM` 백업·백필·`NOT NULL` DDL을 포함한다.
...
- 새 코드 학습 가이드는 MD 1,054줄, HTML 목차 69개·표 13개·접기 영역 4개이며 외부 의존성 0, UTF-8 BOM 없음, 브라우저 page error 0을 확인했다. 문서에 적은 Java 메서드 12개와 MyBatis id 16개가 현재 코드에 존재하고 대상 4파일에 미커밋 diff가 없음을 대조했다.
