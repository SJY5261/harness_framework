...
- [SiteAdmin_재질표면처리매핑] **DB·백엔드·필터·캐시 키 전환·전체 빌드·실화면 연결 검증 완료 / UI·기존 데이터 정책 대기** — 재시작된 8081 서버에서 `g_code`, 브라우저 캐시, `/json-list` 모두 D03 51개의 신규 `MATERIAL_TYPE_CDS`를 반환하고 소재별 필터 건수가 DB 매핑과 일치함을 확인했다. 남은 범위는 Site Admin 편집 UI와 기존 데이터·코드 정본 정책이다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
