...
- [가공확정] **KAN-6 공통 작업상세 표 재구현·격리 빌드 완료 / KAN-63 추론성 재확정 변경 원복, KAN-35 DB 반영 완료, KAN-38 보류** — 사용자와 Jira 범위를 다시 대조해 기존 7파일 변경을 모두 원복하고, 실제 대상인 공통 `bottom.jsp` 데스크톱 표 1파일만 `+4/-8`로 수정했다. 가공확정 일시는 제거하고 작업확정·가공시작·가공완료·진행상태를 빈칸 없이 재배치했으며 데이터 바인딩은 유지했다. 과거 KAN stash는 폐기된 범위를 포함하므로 최종 적용본으로 사용하지 않는다 — [상세](handoff/가공확정.md)
