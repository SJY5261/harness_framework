...
- 2026-08-20 KAN-47 `사이트 설정 > Area 구분`을 KAN-46의 최소 diff·기존 기능 재사용 방식으로 구현했다. Chrome 다운로드 기록의 `selectedIssue=KAN-47`로 Jira 첨부 `p079.jpg`·`p080.jpg`의 이슈 연결을 확인하고, 공용 `selectCommonCodeList(HIGH_CD=E05)`·`commonCodeModifyGrid`·`paramQueryModifyGrid(systemType=CODE)`와 기존 `insertCommonCode`·`insertCommonLangCode`·`deleteCommonCodeLangAdmin`·`deleteCommonCodeAdmin`을 재사용했다. 신규 코드는 `selectAreaCodeUsageCount` 하나뿐이며 로그인 테넌트의 `TBL_EQUIP.FACTORY_AREA`, `TBL_CODE(HIGH_CD=E07).REF_CD`, `TBL_ALARM_DETAIL.FACTORY_AREA` 참조를 합산한다. `TBL_ALARM_DETAIL`은 `SYSTEM_ID`가 없어 생성자 `INSERT_SEQ`를 전역 고유 `TBL_USER.USER_SEQ`에 연결해 로그인 `SYSTEM_ID`로 제한했으며 개발 DB에서 사용자키 51/51 고유, E05 사용 건수 `E05R10=17`, `E05R20=3`, 나머지 0을 읽기 전용 SQL로 확인했다. BASIC 5건은 체크박스·사용여부를 잠그고 직접추가 행은 단일 선택만 허용하며, 사용여부는 사용자 결정대로 행 메모리 `DEL_YN`만 `N↔Y` 전환하고 DB 요청을 하지 않는다. 인증 화면 `1920×889`에서 E05 5건, 8개 헤더 중앙 정렬, 열 합계 1419.9375px, 가로 넘침 0, 추가 팝업 `E05R51`·`SORT 6`, 빈 이름 검증, 탭 왕복 OFF 유지·ON 복귀, 추가 네트워크 요청 0건, JavaScript 오류 0건을 확인했다. XML 파싱·인라인 JavaScript 문법·`git diff --check`·전체 Gradle 빌드를 통과했고 제품 diff는 `system.xml`, `common-code-master-v2.jsp` 2파일 `+312/-33`, CRLF·UTF-8(no BOM)이다. 직접추가 수정은 KAN-46과 동일하게 후속 대기며, 제품 `sunPro/KAN-47` 변경은 미커밋이다.
- 2026-08-20 KAN-47 최초 8081 프로세스에서 삭제 시 `Mapped Statements collection does not contain value for systemMapper.selectAreaCodeUsageCount`가 발생했다. 원인은 JSP 변경 후 09:52에 기동된 기존 Java 프로세스가 새 MyBatis statement를 로드하지 않은 것이었다. 해당 프로세스를 재기동하고 인증 세션에서 임시 `E05R99`를 실제 추가한 뒤 `/json-info` 사용 건수 조회(HTTP 200, `USAGE_COUNT=0`)와 기존 `/paramQueryModifyGrid` 삭제(HTTP 200)를 연속 검증했으며 `TBL_CODE`·`TBL_CODE_LANG` 임시 행 삭제 SQL 실행까지 확인했다. 재기동 시 기존 메일 스케줄러의 `TBL_MAIL_BOX` 상태 UPDATE가 실행되지만 후속 메일 목록은 0건이었고, 현재 8081은 새 매퍼가 로드된 상태로 유지한다.
- 2026-08-20 KAN-46 최종 정리와 원격 push를 완료했다. 부서 직접추가 판별에서 불필요한 `NEW_YN`을 제거하고 `SYSTEM_ID != 'BASIC'`만 사용하며, BASIC 체크박스의 `editable=false`를 기준으로 체크 결과의 `filter(isCustomDept)`와 `check` 이벤트의 BASIC 재검사를 제거했다. 제품 커밋은 `7c6a207`(`KAN-46 부서종류 실기능 연동`)이며 검증 시점 `sunPro/KAN-46`과 `origin/sunPro/KAN-46`이 일치했다. `origin/main` 대비 변경은 `system.xml`, `common-code-master-v2.jsp` 2파일 `+288/-69`이며 XML 파싱·사이트 설정 JavaScript 문법·`git diff --check`·전체 Gradle 빌드를 다시 통과했다. `main`이 5커밋 앞서 있지만 `git merge-tree` 사전 비교에 충돌 표시는 없었다. PR MD·자체완결 HTML은 `C:\Users\User\Desktop\PR\[08.20 KAN-46 부서종류 실기능 연동]`에 생성했으며, 현재 제품 작업 브랜치는 `sunPro/KAN-47`로 전환됐다. 로컬의 `style.css` 변경과 `shell/runDev.local.bat`는 제품 커밋과 PR 변경량에 포함되지 않는다.
- 2026-08-20 KAN-46의 불필요한 공용 변경을 다시 원복했다. `SystemController`·`SystemService`·`SystemServiceImpl`은 HEAD와 같고 `insertCommonCode`도 기존 `#{SYSTEM_ID}`를 유지하며, 추가 팝업이 인증 사용자 `SYSTEM_ID`와 문자열 `SORT_NUM`을 기존 `commonCodeModifyGrid` 행 형식으로 전달한다. `selectCommonCodeList`의 E04 분기도 제거하고, 삭제 버튼과 최종 확인 시점에만 E04 전용 `selectDepartmentCodeUsageCount`를 기존 `/json-info`로 호출한다. 참조 0건이면 기존 `/paramQueryModifyGrid`의 `systemType=CODE` 처리와 `deleteCommonCodeLangAdmin`·`deleteCommonCodeAdmin` queryId를 재사용한다. 이후 E05~E08 등을 공통 조회에 미리 누적하지 않고 실제 참조 관계가 확인된 사이트 설정 기능별 검증 쿼리로 분리한다. 최초 삭제 시도 오류는 08:41에 시작한 서버가 09:42에 추가된 MyBatis statement를 로드하지 못해 발생했으며, 로컬 서버 재시작 후 동일 조회가 정상화됐다. 사용자 요청으로 현재 KAN-46 작업트리의 소재·표면처리 PQ Grid 전환과 실행 코드는 제거해 원래 정적 목업으로 복구하고 부서 종류 변경만 보존했다. 인증 화면 HTTP 200에서 소재·표면처리 런타임 Grid 제거와 부서 Grid 유지를 확인했으며, XML 파싱·부서 스크립트 문법·전체 Gradle 빌드를 통과했고 제품 파일은 CRLF를 유지한다.
...
- 2026-08-19 KAN-46 후속 규칙은 `다중 삭제 불가·단일 삭제만 허용`, `탭 전환 후 이전 그리드 상태 유지`, `기본값 수정·삭제 불가`, `직접추가 행만 발번 규칙에 따라 생성하고 수정·삭제 허용`으로 확정했다. 사용여부는 화면 목업 기능으로만 남기고 `DEL_YN` 저장·실제 적용은 보류한다. 라벨프린터는 별도 검증 없이 사용자가 입력한 값 전체를 DB에 그대로 저장하는 하드코딩 방식을 적용한다. `system.xml`의 `insertCommonCode`·`insertCommonCodeAdmin`은 모두 `#{CODE_CD}`를 그대로 INSERT하고 `selectKey`가 없으며, `insertCommonCodeForSystem`은 `TBL_BASIC_CODE`를 복사할 뿐이라 기존 XML에 개별 기준코드 자동 발번 로직은 없다.
- 2026-08-19 KAN-46 `사이트 설정 > 부서 종류`는 사용자가 확정한 절차로 다시 검증했다. 로컬 목업의 정적 4건 `ST01xx` 표를 `common-code-master.jsp`의 `commonCodeBotColModel`과 동일한 원격 조회(`/getCommonCodeList`, `systemMapper.selectCommonCodeList`, `HIGH_CD=E04`)를 응용한 PQ Grid로 교체했다. 테넌트 `SYSTEM_ID`를 정상 판별하도록 로컬은 `http://smd.localhost:8081/commonCodeMasterV2`를 사용했고, `E04R10~E04R20` 11건(경영지원~시스템관리자)과 `부서 종류` 컬럼을 확인했다. 외부 Chrome 디버깅 탭은 `%클라우드` 최적화 기준인 3번 모니터 `1920×1080`·브라우저 100%에서 운영한다. 원격 정적 표의 컬럼 합계 1433px보다 로컬 본문 가용 폭이 세로 스크롤 때문에 13px 좁은 1420px임을 실측하고, 원격 비율을 유지한 `106.803472/122.178341/183.135904/188.802774/196.885031/156.179562/229.043746/236.971171px`로 비례 축소해 렌더 합계 1419.9375px·가로 넘침 0px을 확인했다. 체크 컬럼은 `cb.select=false`로 행 선택과 분리했고 직접 체크만 상태와 삭제 버튼을 바꾸며, 사용 ON 아이콘은 표시 전용 `is-on`으로 녹색을 복구했다. 추가 버튼의 `+`를 제거하고 JIRA 첨부 `p077.jpg`·`p078.jpg` 기준 부서 추가, 삭제 확인, 사용 건수 12건 삭제 불가 팝업 흐름을 실화면에서 검증했다. 추가 행 `E04R21` 생성·삭제 후 11건 원복, 조회 HTTP 200·JavaScript 예외 0건, `git diff --check`, CRLF·UTF-8(no BOM), 전체 Gradle 빌드를 통과했다. 다만 `/commonCodeMasterV2`는 미리보기 전용이고 현재 E04 응답에는 `USAGE_COUNT`가 없으며 `commonCodeModifyGrid`도 영속 삭제를 처리하지 않으므로 이번 단계의 추가·삭제는 화면 상태까지만 반영한다. 실제 저장·사용 건수 판정·삭제는 SQL 및 API 범위 합의가 필요하다. 제품 로컬 브랜치는 `main` 커밋 `6de53a1`에서 `sunPro/KAN-46`부터 `sunPro/KAN-54`까지 각각 생성했고, 현재 작업 브랜치는 `sunPro/KAN-46`이다.
...
- 2026-08-18 책임자 검토 결과, 테넌트별 D01·D03 사용 여부를 별도 `TBL_TENANT_CODE_USE`로 관리하는 설계 자체를 채택하지 않기로 했다. 정식 처리는 `TBL_CODE`와 `SF_GET_CODE`의 `DEL_YN` 흐름을 함께 변경해야 하지만 9월 클라우드 오픈 범위를 과도하게 확장하므로 이번 KAN-44에서는 구현하지 않는다. 현재 `sunPro/common-code-master`에는 해당 테이블 참조가 0건이고 D01·D03 Grid는 `A.DEL_YN='N'`만 조회해 `USE_YN='Y'`를 읽기 전용으로 표시한다. 개발 DB에 생성된 빈 테이블은 별도 삭제 승인 전까지 미사용 상태로 남긴다. 이 항목이 아래 2026-08-11 구현 기록보다 우선하는 최신 결정이다.
