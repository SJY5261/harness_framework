...
- [KAN-44_소재표면처리기준정보] **D01 소재 PQ Grid 11 실조회·원격 목업 치수 반영 완료 / 필터·테넌트 ON/OFF 저장 대기** — `sunPro/common-code-master`의 `/commonCodeMasterV2` 소재 표를 개발 DB D01 활성 78건 원격 조회 그리드로 교체했다. 원격 정적 목업의 7개 열 너비·헤더 40px·행 34px·글꼴과 여백을 PQ Grid에 반영했고 열 너비 최대 오차는 0.406px이다. 인증 화면에서 API 200·78건·JavaScript 오류 0건을 확인했다. `TBL_TENANT_CODE_USE`는 설계 결정 전이라 이번 조회·표시에는 사용하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
