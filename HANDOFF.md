...
- [SiteAdmin_재질표면처리매핑] **DB·백엔드·필터·캐시 키 전환·전체 빌드 완료 / 서버 재시작 실화면 검증·UI·기존 데이터 정책 대기** — 직접 시드 전 Redis·브라우저 캐시 때문에 기존 `REF_CD`로 후퇴하던 현상을 재현하고 양쪽 키를 `surface-material-map-v1`으로 전환했다. 실행 중인 8081 서버는 재시작하지 않았으며, 재시작 후 D03 51개의 신규 `MATERIAL_TYPE_CDS` 응답과 소재별 표면처리 확인이 필요하다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
