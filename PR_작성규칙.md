...
- `templates/standalone-report.html`의 `<style>` 블록이 자체완결 HTML의 시각 정본이다. 색상, 글자 굵기, 글자 외곽 보강, 여백, 표, 접힘, 반응형, 다크 모드와 인쇄 스타일을 그대로 인라인으로 포함한다.
- 사용자 요청 없이 작업별로 공통 스타일을 재설계하거나 덮어쓰지 않는다. 문서 내용에만 필요한 추가 스타일은 범위를 한정하고 공통 색상 토큰·타이포그래피·레이아웃을 변경하지 않는다.
...
- [ ] 사용자 지정 예외가 없다면 HTML의 공통 스타일이 `templates/standalone-report.html`의 시각 정본과 일치한다.
