...
- [KAN-44_소재표면처리기준정보] **D01 소재·D03 표면처리 PQ Grid 11 실조회·원격 목업 치수 반영 완료 / 대분류 표시명 설계·필터·테넌트 ON/OFF 저장 대기** — `sunPro/common-code-master`의 `/commonCodeMasterV2`에서 소재 D01 활성 78건과 표면처리 D03 활성 51건을 원격 조회 그리드로 교체했다. 표면처리 7개 열은 원격 DOM 실측값 `83.219/182.109/164.313/267.234/154.500/407.672/173.953px`를 적용해 렌더 오차 0px이며, `TBL_MATERIAL_SURFACE_MAP` 집계로 적용소재를 `공통 24 / AL 전용 24 / Steel 전용 3`으로 표시한다. D01·D03 `ABBR_NM`의 한국어 표시명 정본이 없어 코드값을 유지하고 후속 협의 전까지 변경을 보류했다. 인증 화면에서 두 그리드 API 200·JavaScript 오류 0건을 확인했다. `TBL_TENANT_CODE_USE`는 설계 결정 전이라 이번 조회·표시에는 사용하지 않았다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
...
- [테넌트기준코드사용여부] **개발 DB·V2 조회·저장 API·기존 캐시 무효화 연결 완료 / 실화면 검증 대기** — `TBL_TENANT_CODE_USE`의 D01·D03 `USE_YN`을 V2 화면에 연결했고, 신규 탭 전파 대신 기존 `clearAll`·`BroadcastChannel`과 Redis 전체 clear를 재사용하도록 단순화했다. 제품 변경은 tracked 8파일과 재현 SQL 1파일이며 전체 빌드와 JS·diff 검증을 통과했다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
