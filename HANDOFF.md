...
- [가공확정] **R14/R16 생성·판정 의존 제거·작업상세 배지 적용 / NULL 후속작업 취소 정상 복귀 수정·실서버 검증 / DB 이관 보류** — CAM·MCT 취소 시 최근 작업확정 이후 유효한 파트 이력만 복귀 대상으로 사용하고, 이력이 없으면 `PART_STATUS=NULL`로 복귀하도록 수정했다. 실제 8081 서버에서 CAM `NULL → S53R26 → NULL`, MCT `NULL → S53R28 → NULL`을 확인했고 재확정 이전의 과거 상태는 복원하지 않으며 작업확정 이후의 유효 상태는 보존한다 — [상세](handoff/가공확정.md)
