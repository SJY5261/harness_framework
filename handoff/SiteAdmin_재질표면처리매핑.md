...
- 2026-08-20 KAN-47 신규 MyBatis statement의 재기동 검증을 위해 18081 분리 서버를 기동했으나 기존 메일 스케줄러가 `TBL_MAIL_BOX` 상태 UPDATE를 실행했다. 즉시 후속 조회 결과 행이 없어 실제 변경은 0건으로 보이지만 영향 행 수가 로그에 없어 확정할 수는 없다. 중복 스케줄 위험을 막기 위해 분리 서버를 바로 종료하고 18081 포트 해제를 확인했으며, 기존 8081은 중단하지 않았다. 따라서 신규 삭제 statement는 XML 파싱·빌드·동일 SQL의 직접 읽기 조회까지만 통과했고 재기동한 애플리케이션의 `/json-info` 실호출과 실제 추가·삭제 DB 왕복은 미실행이다. 스케줄러 억제 방식이 확정되기 전에는 검증용 두 번째 애플리케이션 인스턴스를 띄우지 않는다.
