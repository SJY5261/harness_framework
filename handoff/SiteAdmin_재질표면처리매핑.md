...
- 2026-08-19 KAN-46 `사이트 설정 > 부서 종류`는 사용자가 확정한 절차로 다시 검증했다. 로컬 목업의 정적 4건 `ST01xx` 표를 `common-code-master.jsp`의 `commonCodeBotColModel`과 동일한 원격 조회(`/getCommonCodeList`, `systemMapper.selectCommonCodeList`, `HIGH_CD=E04`)를 응용한 PQ Grid로 교체했다. 테넌트 `SYSTEM_ID`를 정상 판별하도록 로컬은 `http://smd.localhost:8081/commonCodeMasterV2`를 사용했고, `E04R10~E04R20` 11건(경영지원~시스템관리자)과 `부서 종류` 컬럼을 확인했다. 외부 Chrome 디버깅 탭에 `https://dev-smd.tomes.co.kr:8081/commonCodeMasterV2`와 로컬을 동시에 열고 같은 1707×876 뷰포트에서 원격 정적 표의 8개 열을 `91.6667/104.9792/157.7396/162.6563/169.6458/134.4063/195.8854/204.3542px`로 실측했다. 로컬 PQ Grid에 같은 폭을 적용한 결과 컬럼 합계는 양쪽 모두 1221.33px, 열별 최대 오차는 0.0104px이다. 실화면 E04 11건·조회 HTTP 200·JavaScript 예외 0건, `git diff --check`, CRLF·UTF-8(no BOM), `%클라우드` 전체 Gradle 빌드를 통과했다. SQL은 변경하지 않았으며 추가·수정·삭제 등 부가기능은 사용자가 지적하는 항목만 순차적으로 진행한다.
