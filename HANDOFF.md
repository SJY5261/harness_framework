...
- [가공확정] **R14/R16 생성·판정 의존 제거·작업상세 배지 적용 / NULL 후속작업 취소 정상 복귀 수정·실서버 검증 / 과거 KAN-6·63 stash 폐기 / DB 이관 보류** — CAM·MCT 취소 시 최근 작업확정 이후 유효한 파트 이력만 복귀 대상으로 사용하고, 이력이 없으면 `PART_STATUS=NULL`로 복귀하도록 수정했다. 실제 8081 서버에서 CAM `NULL → S53R26 → NULL`, MCT `NULL → S53R28 → NULL`을 확인했다. 과거 KAN-6 다중 화면 변경·KAN-63 재확정 예외와 그 통합본 stash 3개는 사용자 지시에 따라 삭제했다 — [상세](handoff/가공확정.md)
