...
- [KAN-46_사이트설정부서종류] **화면설계 상호작용 완료 / DB 영속화 범위 미정의로 제외** — Jira와 일치하는 `p077.jpg`·`p078.jpg`를 기준으로 `/commonCodeMasterV2` 부서 종류 목록의 동적 건수, `ST01xx` 자동 코드 추가, 더블클릭 수정, 직접추가 행 강조, 사용여부, 선택·전체 선택, 미사용 행 삭제 확인과 사용 12건 행 삭제 차단을 구현했다. 1600×1000 인증 화면에서 전체 흐름과 JavaScript 오류 0건, 전체 Gradle 빌드를 확인했다 — [상세](handoff/SiteAdmin_재질표면처리매핑.md)
