...
- 2026-08-20 KAN-46 최종 정리와 원격 push를 완료했다. 부서 직접추가 판별에서 불필요한 `NEW_YN`을 제거하고 `SYSTEM_ID != 'BASIC'`만 사용하며, BASIC 체크박스의 `editable=false`를 기준으로 체크 결과의 `filter(isCustomDept)`와 `check` 이벤트의 BASIC 재검사를 제거했다. 제품 커밋은 `7c6a207`(`KAN-46 부서종류 실기능 연동`)이고 `origin/sunPro/KAN-46`과 로컬 HEAD가 일치한다. `origin/main` 대비 변경은 `system.xml`, `common-code-master-v2.jsp` 2파일 `+288/-69`이며 XML 파싱·사이트 설정 JavaScript 문법·`git diff --check`·전체 Gradle 빌드를 다시 통과했다. `main`이 5커밋 앞서 있지만 `git merge-tree` 사전 비교에 충돌 표시는 없었다. PR MD·자체완결 HTML은 `C:\Users\User\Desktop\PR\[08.20 KAN-46 부서종류 실기능 연동]`에 생성했으며, 현재 로컬의 `style.css` 변경과 `shell/runDev.local.bat`는 제품 커밋과 PR 변경량에 포함되지 않는다.
